...
const TIPCLUB = require('../pages/TIPCLUB');
...
test('Check Register and navigate to game screen for all URLs', async ({page}) => {
  ...
  for (const url of urls) {
    const tipclub = new TIPCLUB(page);

    try {
      console.log(`Đang điều hướng đến: ${url}`);
      await tipclub.navigate(url);
      ...
      await tipclub.register(username, password, phone);

      // Verify tài khoản thành công và chuyển sang màn hình game
      await tipclub.verifyDangKyThanhCongTIPCLUB(url);
